...
const Navbar = () => {
    ...
    return (
        <>
    <nav>
        <section className='nav1 h-full px-3 logo-div relative  flex justify-between items-center py-4 font-medium'>
            <div className=" nav1-logo flex justify-start items-center ps-5">
                <img className='w-36' src={assets.logo} alt="Logo" />
            </div>
            <ul className={`nav-links h-svh absolute ${ showMenu ? "flex" : "hidden"} shadow-inner flex-col right-0 w-[60%] right-0 bg-[#fac5c5] md:bg-white rounded p-10 top-16 md:flex gap-5 md:h-0 md:w-auto md:flex-row md:top-0 md:right-0 md:relative text-[16px] text-white md:text-sm md:text-gray-700 md:shadow-none`}>
                <NavLink className='flex flex-col gap-1 items-left md:items-center  ' to='/'>
                ...
                <NavLink className='flex flex-col gap-1 items-left md:items-center  ' to='/collections'>
                    <p className='text-white md:text-black text-[16px] align-left'>Collections</p>
                    <hr className="nav-under w-full md:w-3/4 h-[2px] md:bg-gray-700 hidden"></hr>
                </NavLink>
                <NavLink className='flex flex-col gap-1 items-left md:items-center ' to='/aboutUs'>
