...
function ProductCard({product, assets}) {
  ...
  return (
    <>
    <Link  className='product-card  cursor-pointer rounded border flex flex-col h-full ' key={product._id}>
        {/* <div className="card flex flex-col justify-center h-full items-center p-3 ">     */}
            <div className='overflow-hidden'>
                <img src={assets[product.image]} alt={product.name} className=" hover:scale-105 transition ease-in-out product-image rounded w-full h-auto " />
            </div>
